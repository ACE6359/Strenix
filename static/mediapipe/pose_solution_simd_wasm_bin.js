var createMediapipeSolutionsWasm = (() => {
	...
	return function (createMediapipeSolutionsWasm) {
		...
		var wasmBinaryFile;
		wasmBinaryFile = "pose_solution_simd_wasm_bin.wasm";
					...
					findCanvasEventTarget = function (target) {
						if (target == 0) {
							if (Module && Module.canvas) {
								...
								return cachedFindCanvasEventTarget(
		...
		function mediapipe_import_external_texture(
			device_handle,
			source_handle,
		) {
		...
		function registerInheritedInstance(class_, ptr, instance) {
			...
			if (registeredInstances.hasOwnProperty(ptr)) {
				throwBindingError(
					"Tried to register registered instance: " + ptr,
				);
			} else {
		...
		function __emval_get_method_caller(argCount, argTypes) {
			...
			var invokerFunction = (handle, name, destructors, args) => {
				...
				for (var i = 0; i < argCount - 1; ++i) {
					argN[i] = types[i + 1]["readValueFromPointer"](
						args + offset,
					);
		...
		function _emscripten_stack_unwind_buffer(addr, buffer, count) {
			...
			for (var i = 0; i < count && stack[i + offset]; ++i) {
				HEAP32[(buffer + i * 4) >> 2] = convertFrameToPC(
					stack[i + offset],
				);
			}
			return i;
		...
		function __webgl_enable_WEBGL_multi_draw(ctx) {
			return !!(ctx.multiDrawWebgl =
				ctx.getExtension("WEBGL_multi_draw"));
		}
		var GL = {
			counter: 1,
			buffers: [],
			...
			generateTempBuffers: function (quads, context) {
				...
				context.tempVertexBuffers1.length =
					context.tempVertexBuffers2.length = largestIndex + 1;
			...
			enableOffscreenFramebufferAttributes: function (
				webGLContextAttributes,
			) {
			...
			createOffscreenFramebuffer: function (context) {
				...
				if (gl.getContextAttributes().antialias) {
					...
					if (firefoxMatch != null) {
						...
						context.defaultFboForbidBlitFramebuffer =
							firefoxVersion < 67;
					}
				}
				...
				if (
					...
					if (context.defaultVao) {
						...
						var prevVertexAttribPointer = {
							...
							pointer: gl.getVertexAttribOffset(
								context.blitPosLoc,
								34373,
							),
						};
		...
		var _emscripten_webgl_create_context =
		...
		function _emscripten_webgl_get_context_attributes(c, a) {
			...
			var power =
				t["powerPreference"] &&
				__emscripten_webgl_power_preferences.indexOf(
					t["powerPreference"],
				);
			HEAP32[(a + 24) >> 2] = power;
		...
		var WebGPU = {
			initManagers: function () {
				...
				WebGPU.mgrCommandBuffer =
					WebGPU.mgrCommandBuffer || new Manager();
				WebGPU.mgrCommandEncoder =
			...
			makeTextureDataLayout: function (ptr) {
				...
				return {
					offset:
						HEAPU32[(ptr + 4 + 8) >> 2] * 4294967296 +
						HEAPU32[(ptr + 8) >> 2],
					bytesPerRow:
						bytesPerRow === 4294967295 ? undefined : bytesPerRow,
					rowsPerImage:
						rowsPerImage === 4294967295 ? undefined : rowsPerImage,
				};
			},
		...
		function writeI53ToI64(ptr, num) {
			...
			var deserialized =
				num >= 0 ? readI53FromU64(ptr) : readI53FromI64(ptr);
			if (deserialized != num)
				warnOnce(
				...
				if (!program.maxUniformBlockNameLength) {
					for (
						...
						program.maxUniformBlockNameLength = Math.max(
							program.maxUniformBlockNameLength,
							GLctx.getActiveUniformBlockName(program, i).length +
								1,
						);
					}
				}
		...
		function _wgpuBufferGetMappedRange(bufferId, offset, size) {
			...
			if (size === 0)
				warnOnce(
					"getMappedRange size=0 no longer means WGPU_WHOLE_MAP_SIZE",
				);
			size = size >>> 0;
			if (size === 4294967295) size = undefined;
			if (bufferWrapper.mapMode !== 2) {
				abort(
					"GetMappedRange called, but buffer not mapped for writing",
				);
				return 0;
			}
			var mapped;
		...
		function _wgpuCommandEncoderBeginComputePass(encoderId, descriptor) {
			...
			function makeComputePassTimestampWrite(twPtr) {
				return {
					...
					location:
						WebGPU.ComputePassTimestampLocation[
							HEAPU32[(twPtr + 8) >> 2]
						],
				};
			}
			function makeComputePassTimestampWrites(count, twPtr) {
				...
				for (var i = 0; i < count; ++i) {
					timestampWrites.push(
						makeComputePassTimestampWrite(twPtr + 12 * i),
					);
				}
		...
		function _wgpuCommandEncoderBeginRenderPass(encoderId, descriptor) {
			...
			function makeRenderPassTimestampWrite(twPtr) {
				return {
					...
					queryIndex: HEAPU32[(twPtr + 4) >> 2],
					location:
						WebGPU.RenderPassTimestampLocation[
							HEAPU32[(twPtr + 8) >> 2]
						],
				};
			}
			function makeRenderPassTimestampWrites(count, twPtr) {
				...
				for (var i = 0; i < count; ++i) {
					timestampWrites.push(
						makeRenderPassTimestampWrite(twPtr + 12 * i),
					);
				}
				return timestampWrites;
			}
			function makeRenderPassDescriptor(descriptor) {
				...
				if (nextInChainPtr !== 0) {
					...
					assert(
						HEAPU32[renderPassDescriptorMaxDrawCount >> 2] === 0,
					);
					maxDrawCount =
						HEAPU32[
							(renderPassDescriptorMaxDrawCount + 4 + 8) >> 2
						] *
							4294967296 +
		...
		function _wgpuComputePipelineGetBindGroupLayout(
			pipelineId,
			groupIndex,
		) {
			var pipeline = WebGPU.mgrComputePipeline.get(pipelineId);
		...
		function _wgpuDeviceCreateRenderPipeline(deviceId, descriptor) {
			...
			function makePrimitiveState(rsPtr) {
				...
				return {
					topology:
						WebGPU.PrimitiveTopology[HEAPU32[(rsPtr + 4) >> 2]],
					stripIndexFormat:
						WebGPU.IndexFormat[HEAPU32[(rsPtr + 8) >> 2]],
			...
			function makeStencilStateFace(ssfPtr) {
				...
				return {
					...
					depthFailOp:
						WebGPU.StencilOperation[HEAPU32[(ssfPtr + 8) >> 2]],
			...
			function makeDepthStencilState(dssPtr) {
				...
				return {
					...
					depthCompare:
						WebGPU.CompareFunction[HEAPU32[(dssPtr + 12) >> 2]],
		...
		function _wgpuDeviceCreateSampler(deviceId, descriptor) {
			...
			var desc = {
				label: undefined,
				addressModeU:
					WebGPU.AddressMode[HEAPU32[(descriptor + 8) >> 2]],
				addressModeV:
					WebGPU.AddressMode[HEAPU32[(descriptor + 12) >> 2]],
				addressModeW:
					WebGPU.AddressMode[HEAPU32[(descriptor + 16) >> 2]],
				magFilter: WebGPU.FilterMode[HEAPU32[(descriptor + 20) >> 2]],
				minFilter: WebGPU.FilterMode[HEAPU32[(descriptor + 24) >> 2]],
				mipmapFilter:
					WebGPU.FilterMode[HEAPU32[(descriptor + 28) >> 2]],
				lodMinClamp: HEAPF32[(descriptor + 32) >> 2],
				lodMaxClamp: HEAPF32[(descriptor + 36) >> 2],
				compare:
					WebGPU.CompareFunction[HEAPU32[(descriptor + 40) >> 2]],
			};
			var labelPtr = HEAPU32[(descriptor + 4) >> 2];
			if (labelPtr) desc["label"] = UTF8ToString(labelPtr);
			var device = WebGPU.mgrDevice.get(deviceId);
		...
		function _wgpuDeviceCreateShaderModule(deviceId, descriptor) {
			...
			var device = WebGPU.mgrDevice.get(deviceId);
			return WebGPU.mgrShaderModule.create(
		...
		function _wgpuDeviceCreateTexture(deviceId, descriptor) {
			...
			var desc = {
				...
				dimension:
					WebGPU.TextureDimension[HEAPU32[(descriptor + 12) >> 2]],
		...
		function _wgpuTextureCreateView(textureId, descriptor) {
			...
			if (descriptor) {
				...
				desc = {
					...
					mipLevelCount:
						mipLevelCount === 4294967295
							? undefined
							: mipLevelCount,
					baseArrayLayer: HEAPU32[(descriptor + 24) >> 2],
					arrayLayerCount:
						arrayLayerCount === 4294967295
							? undefined
							: arrayLayerCount,
		...
		for (var i = 0; i < 288; ++i) {
			miniTempWebGLFloatBuffers[i] =
				miniTempWebGLFloatBuffersStorage.subarray(0, i + 1);
		}
		var __miniTempWebGLIntBuffersStorage = new Int32Array(288);
		for (var i = 0; i < 288; ++i) {
			__miniTempWebGLIntBuffers[i] =
				__miniTempWebGLIntBuffersStorage.subarray(0, i + 1);
		}
		var decodeBase64 =
		...
		var asmLibraryArg = {
			...
			emscripten_webgpu_import_bind_group:
				_emscripten_webgpu_import_bind_group,
			emscripten_webgpu_import_texture: _emscripten_webgpu_import_texture,
			emscripten_webgpu_release_js_handle:
				_emscripten_webgpu_release_js_handle,
			...
			mediapipe_create_utility_canvas2d:
				mediapipe_create_utility_canvas2d,
			mediapipe_find_canvas_event_target:
				_mediapipe_find_canvas_event_target,
			mediapipe_import_external_texture:
				mediapipe_import_external_texture,
			mediapipe_webgl_tex_image_drawable:
				_mediapipe_webgl_tex_image_drawable,
		...
		var dynCall_vj = (Module["dynCall_vj"] =
			createExportWrapper("dynCall_vj"));
		...
		var ___start_em_js = (Module["___start_em_js"] = 607347);
		var ___stop_em_js = (Module["___stop_em_js"] = 607867);
